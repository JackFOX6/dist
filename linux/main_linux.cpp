...
if (ImGui::CollapsingHeader("STATUS", ImGuiTreeNodeFlags_DefaultOpen)) {
  ...
  if (!JNIHelper::g_playerInstance && jniOk) {
...
__attribute__((constructor)) static void safemode_constructor() {
  fLog = fopen("/home/j4ck/Dev/Prototipo-2.1 (Limpio)/logs/zomboid_safemode_linux.log", "w");
  Log("=== [Prototipo-SafeMode] Linux LD_PRELOAD ===");
...
__attribute__((destructor)) static void safemode_destructor() {
  if (g_imguiInitialized) {
