    ...
    for (int dy = -radiusTiles; dy <= radiusTiles; dy++) {
      ...
      if (square) {
         if (g_midGridSquareGetStaticMovingObjects) {
             ...
             if (objList) {
                 int size = env->CallIntMethod(objList, g_midArrayListSize);
                 if (env->ExceptionCheck()) env->ExceptionClear();
                 
                 for (int i = 0; i < size; i++) {
                     jobject obj = env->CallObjectMethod(objList, g_midArrayListGet, i);
                     if (env->ExceptionCheck()) env->ExceptionClear();
...
std::vector<std::string> JNIHelper::GetAllItems() {
  ...
  int size = env->CallIntMethod(arrayList, g_midArrayListSize);
  for (int i = 0; i < size; i++) {
    jobject itemObj = env->CallObjectMethod(arrayList, g_midArrayListGet, i);
    if (itemObj) {
      std::string fullInfo;
      jstring jFullName =
...
std::vector<JNIHelper::LootInfo> JNIHelper::GetNearbyLoot(int radiusTiles) {
  ...
  if (!g_midArrayListSize || !g_midArrayListGet) return result;
  ...
  // Obtener IsoWorld → IsoCell → chunk squares
  // Método simplificado: usar el IsoCell cached vía IsoWorld.getInstance().getCell()
  // [R-LOWSPEC] IDs cacheados como static — FindClass/GetMethodID solo en primera llamada.
  // Las llamadas subsiguientes (cada 120 frames) reutilizan los IDs directamente.
  static jmethodID s_midGetInstance = nullptr;
  static jmethodID s_midGetCell     = nullptr;
  static jmethodID s_midGetSquare   = nullptr;

  // Usar clase global cacheada por Initialize() en lugar de FindClass cada vez.
  ...
  if (!s_midGetSquare) {
    ...
    // IsoCell.getGridSquare(int x, int y, int z) -> IsoGridSquare
  ...
  for (int dx = -radiusTiles; dx <= radiusTiles && scanned < 800; dx++) {
    for (int dy = -radiusTiles; dy <= radiusTiles && scanned < 800; dy++) {
      ...
      jint objCount = env->CallIntMethod(objList, g_midArrayListSize);
      env->ExceptionClear();

      for (jint oi = 0; oi < objCount && oi < 20; oi++) {
        jobject isoObj = env->CallObjectMethod(objList, g_midArrayListGet, oi);
        ...
        jint itemCount = env->CallIntMethod(items, g_midArrayListSize);
        env->ExceptionClear();
        ...
        for (jint ii = 0; ii < itemCount && ii < 8; ii++) {
          jobject item = env->CallObjectMethod(items, g_midArrayListGet, ii);
            ...
            if (jname) {
              ...
              if (chars) { 
                ...
                if (g_lootFilterCustom[0] != '\0') {
                    ...
                    if (containsIgnoreCase(lItem.name, g_lootFilterCustom) || 
                        containsIgnoreCase(lItem.category, g_lootFilterCustom)) {
                        pCustom = true;
                    }
...
std::string JNIHelper::GetNearVehicleScriptName() {
  ...
  if (vehicleList) {
    jmethodID midToArray = env->GetMethodID(g_cachedArrayListClass, "toArray", "()[Ljava/lang/Object;");
    jmethodID arrMethod = g_midSetToArray ? g_midSetToArray : midToArray;
    jobjectArray vehicleArray = (jobjectArray)env->CallObjectMethod(vehicleList, arrMethod);
