...
#define JNI_LOG(fmt, ...) \
    do { \
        if (fLog) { \
            fprintf(fLog, fmt "\n", ##__VA_ARGS__); \
            /* [R-LOWSPEC] Sin fflush aqui — syscall bloqueante en render thread. */ \
            /* El OS buffer flush ocurre al cerrar el archivo (hack_destructor). */ \
        } \
        printf(fmt "\n", ##__VA_ARGS__); \
    } while(0)
